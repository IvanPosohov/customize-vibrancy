	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		try {
			PackageInfo pinfo = getPackageManager().getPackageInfo(
					getPackageName(), 0);
			text_version.setText(String.format(
					getString(R.string.about_version), pinfo.versionName));
		...
		TransformFilter addSubjFilter = new TransformFilter() {
			public final String transformUrl(final Matcher match, String url) {
				return url + "?subject=" + getString(R.string.app_name)
						+ " feedback"; // &body=message text
			}
		};
		Linkify.addLinks(text_feedback, EMAIL_ADDRESS_PATTERN, "mailto:", null,
				addSubjFilter);
	}
}